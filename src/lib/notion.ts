...
export async function getNotionProducts() {
    ...
    console.log('getNotionProducts: fetching active products for DB:', databaseId);
    
    if (!NOTION_SECRET || !databaseId) return [];

    try {
        const checkboxName = locale === 'en' ? 'Archive' : 'Архив';
...
export async function getNotionArchiveProducts() {
    const locale = await getLocale();
    const databaseId = locale === 'en' ? process.env.NOTION_DATABASE_EN_ID : process.env.NOTION_DATABASE_ID;

    console.log('getNotionArchiveProducts: fetching archive products for DB:', databaseId);
    ...
    try {
        ...
        console.error('Error in getNotionArchiveProducts:', error.message);
        return [];
    }
}
...
export async function getNotionArchiveMainPageData() {
    return null;
}