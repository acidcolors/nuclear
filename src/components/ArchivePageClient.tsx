...
export default function ArchivePageClient({ initialProducts, initialHeader, forcedLoading = false }: ArchivePageClientProps) {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                ...
                if (isMounted) setIsDataLoaded(true); // Все равно пускаем дальше
            }
        }
    ...
    const headerTitle = (rawTitle && rawTitle !== 'Продукты' && rawTitle !== 'Архив') ? rawTitle : t('title');
    const headerDescription = (CMS_CONFIG.USE_NOTION && headerData?.description) ? headerData.description : "Здесь собраны наши архивные работы. По любым вопросам — пишите в директ.";
    ...
    return (
        <main id="archive-main-container" className={`fixed top-0 left-0 w-full h-[100dvh] bg-[#efefef] text-[#111] z-[60] ${isDesktop ? 'overflow-hidden' : 'overflow-y-auto overflow-x-hidden'}`}>
            ...
            <div
                ...
                onWheel={isDesktop ? (e) => {
                    const scrollDiv = document.getElementById('archive-scroll-track');
