...
## 3. Маршрутизация уведомлений (Telegram Forum)
- **Chat ID группы:** `-1003811463175`
- **Логика топиков:**
  - **Сайт + Телефон:**
    - Заказы -> Топик **#3**
    - Поддержка -> Топик **#5**
  - **Сайт + @username** ИЛИ **Приложение (source: 'app')**:
    - Создается **новый персональный топик** через `createForumTopic`.
- **Утилиты:** Все функции собраны в `src/lib/telegram.ts`.

## 4. Notion CMS
- **База заказов:** `NOTION_ORDERS_DB_ID` (из .env)
- **Логика:** При каждом оформлении заказа (кроме типа `support`) данные дублируются в Notion через `createNotionOrder`.

## 5. Контакты пользователя
- Используется единое поле "Telegram или Телефон".
